  ...
  B_1 = c(24.0, 29.8) # Восточная звезда, до наблюдения
  A_1 = c(26.8, 36.7) # Восточная звезда, после наблюдения
  ...
  T_E_1 = c(18,   18,   18, 18,  18)
  T_E_2 = c(56,   57,   57, 58,  58)
  T_E_3 = c(53.5, 17.5, 50, 8.5, 34.5) #+ 503.88
  
  T_E_dec = T_E_1 * 15 + T_E_2 / 4 + T_E_3 / 240
  
  T_W_1 = c(19, 19, 19, 19,   18, 19)
  T_W_2 = c(4,  5,  5,  5,    6,  6)
  T_W_3 = c(45, 5, 32, 49.5, 22, 43.5) #+ 503.88
  
  T_W_dec = T_W_1 * 15 + T_W_2 / 4 + T_W_3 / 240
  
  show_in_the_hour_system_from_dec(1 / 2 * (T_W_dec[2:5] + T_E_dec[2:5]))
  show_in_the_hour_system_from_dec(1 / 2 * (T_W_dec[2:5] - T_E_dec[2:5]))
  ...
  A_1 = mean(1 / 2 * (T_W_dec[2:5] + T_E_dec[2:5]))
  A_2 = mean(1 / 2 * (T_W_dec[2:5] - T_E_dec[2:5]))
  ...
  alpha = transfer_from_hour_system_to_dec(c(18, 57, 54.276)) # в часовой
  delta = c(30,  56,  19.67)  # в градусной
  eps   = c( 0,  37,  58.89)  # в градусной
  beta  = c( 2,  15,  52.354) # в часовой
  ...
  A_W = transfer_from_degree_system_to_dec(c(56, 30, 0))
  ...
  z = transfer_from_degree_system_to_dec(c(37, 2, 0))
