  ...
  M_north = c(15.055, 15.040, 15.010, 15.015, 15.040, 15.022, 15.025)
  Mn = mean(M_north)
  Mn
  
  M_south = c(5.525, 5.550, 5.500, 5.530, 5.550, 5.530, 5.549)
  Ms = mean(M_south)
  Ms
  ...
  JIuIIn = mean(c(69.0 + 83.6, 69.0 + 83.5))
  JIuIIn 
  
  JIuIIs = mean(c(71.2 + 85.8, 71.0 + 85.5))
  JIuIIs
  ...
  thr_num_2 = 7
    ...
    # Подвожу к результату Витязева, так как нет данных по нитям для инструмента 112104
  ...
  # Блок с редукционными величинами
    ...
    # Подставляю результаты Витязева за неимением данных
  ...
  # посчитанные на эпоху наблюдений. Поэтому здесь (в частности) пример Витязева
  # даёт немного неверный результат.
    
  if (sign_val == 1) {
    
    d0 = 0.5 * (deltaS_dec + deltaN_dec)
    
  } else {
    
    d0 = 90 + 0.5 * (deltaS_dec - deltaN_dec)
