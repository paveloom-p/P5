  ...
  # Значения азимутов для наблюдений Полярной звезды:
  ...
  # Табличные значения для альфа (между дробными сутками 1.3 и 2.3 для Июля 2019-го) 
  # с переводом в десятичные углы (запись в виде (часы, минуты, секунды))
  
  alpha_1 = c(2, 55, 2.59)
  alpha_2 = c(2, 55, 4.59)
  
  alpha_1_dec = alpha_1[1] * 15 + alpha_1[2] / 4 + alpha_1[3] / 240
  alpha_1_dec
  
  alpha_2_dec = alpha_2[1] * 15 + alpha_2[2] / 4 + alpha_2[3] / 240
  alpha_2_dec
  ...
  delta_1 = c(89, 20, 22.61)
  delta_2 = c(89, 20, 22.45)
  
  delta_1_dec = delta_1[1] + delta_1[2] / 60 + delta_1[3] / 3600
  delta_1_dec
  
  delta_2_dec = delta_2[1] + delta_2[2] / 60 + delta_2[3] / 3600
  delta_2_dec
  
  # Интерполяция для дельта
  
  delta = delta_1_dec + (delta_1_dec - delta_2_dec) * n
  delta
